	...
	return `<div class="${"wrapper svelte-1cmpm85"}">${ `<form class="${"svelte-1cmpm85"}"><div class="${"form-group"}"><label for="${"exampleInputEmail1"}">Name</label>
		...
		: ``}</div>
			...
			default: () => `Already Registered?
                ...
                <button type="${"button"}" class="${"btn btn-primary"}">Register</button></div></form>`
	}</div>`;
});
...
const Profile = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	...
	profileService.getUserProfile().then(data => profile = data === null || data === void 0 ? void 0 : data.user).catch(e => console.warn(e));
	...
	})(profileService.getUserProfile())}</div></div>`;
});
...
const Verify = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
	const authService = AuthService.getInstance();
	let isLoading = true;
	let isError = false;
	let errorMsg = "";
	...
	return `<div class="${"wrapper"}">${isLoading ? `Loading ...` : ``}
	...
	? `Your account has been successfuly verified. <br>
        Go to ${validate_component(Link, "Link").$$render($$result, { to: "/login" }, {}, { default: () => `Login Page` })}`
	: ``}</div>`;
});
...
const App = create_ssr_component(($$result, $$props, $$bindings, $$slots) => {
    ...
    ${validate_component(Route, "Route").$$render($$result, { path: "verify", component: Verify }, {}, {})}`
	})}`;
});
